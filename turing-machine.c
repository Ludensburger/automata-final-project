...
int getSymbolIndex(char symbol) {
    ...
    case '_':
    case ' ':
        return 4; // Treat both '_' and ' ' as BLANK
    default:
        return -1;
    }
}
...
void initializeStates(State states[]) {
    ...
    states[2].transitions[4] = (Transition){4, '_', RIGHT}; // BLANK -> Hang

    // q3 transitions
    states[3].transitions[0] = (Transition){4, 'a', LEFT};  // a -> q4, a, L
    states[3].transitions[1] = (Transition){5, 'A', RIGHT}; // A -> q5, A, R
    states[3].transitions[2] = (Transition){-1, '_', HANG}; // b -> Hang
    states[3].transitions[3] = (Transition){3, 'B', LEFT};  // B -> q3, B, L
    states[3].transitions[4] = (Transition){-1, '_', HANG}; // BLANK -> Hang
    ...
    states[7].transitions[4] = (Transition){9, '_', RIGHT}; // BLANK -> Hang

    // q8 transitions
    states[8].transitions[0] = (Transition){-1, '_', HANG};  // a -> Hang
    ...
    states[8].transitions[4] = (Transition){-1, '_', HANG};  // BLANK -> Hang
    ...
    states[9].transitions[4] = (Transition){12, '_', RIGHT}; // BLANK -> Halt
    ...
    states[10].transitions[3] = (Transition){-1, '_', HALT};  // BLANK -> Halt
    ...
    states[11].transitions[3] = (Transition){-1, '_', HANG};  // B -> Hang
    states[11].transitions[4] = (Transition){12, '_', LEFT};  // BLANK -> q12, BLANK, L

    // q12 transitions
    states[12].transitions[0] = (Transition){-1, '_', HANG}; // a -> Hang
    states[12].transitions[1] = (Transition){1, 'A', RIGHT}; // A -> q1, A, R
    states[12].transitions[2] = (Transition){-1, '_', HANG}; // b -> Hang
    states[12].transitions[3] = (Transition){1, 'B', RIGHT}; // B -> q1, B, R
    states[12].transitions[4] = (Transition){-1, '_', HALT}; // BLANK -> Halt
}
...
int simulateTuringMachine(State states[], char **tape, int startState) {
    ...
    int head = 0;
    int tapeSize = INITIAL_TAPE_SIZE;
    ...
    while (currentState != -1) { // -1 represents "Hang"
        ...
        if (transition.moveDirection == HALT) {
            ...
            if (currentState == 1 && currentSymbol == '_') {
                printf("Tape Rejected! Encountered Hang.\n");
                return 0;

            } else if (currentState == 2 && currentSymbol == '_') {
                printf("Tape Rejected! Encountered Hang.\n");
                return 0;

            } else if (currentState == 3 && currentSymbol == 'b' || currentSymbol == '_') {
                printf("Tape Rejected! Encountered Hang.\n");
                return 0;

            } else if (currentState == 4 && currentSymbol == '_') {
                printf("Tape Accepted!\n");
                return 1;

            } else if (currentState == 5 && currentSymbol == '_') {
                printf("Tape Accepted!\n");
                return 1;
            ...
            } else if (currentState == 7 && currentSymbol == '_') {
                printf("Tape Rejected! Encountered Hang.\n");
                return 0;

            } else if (currentState == 8 && currentSymbol == 'a' || currentSymbol == '_') {
                printf("Tape Rejected! Encountered Hang.\n");
                return 0;

            } else if (currentState == 9 && currentSymbol == '_') {
                printf("Tape Accepted!\n");
                return 1;

            } else if (currentState == 10 && currentSymbol == '_') {
                printf("Tape Accepted!\n");
                return 1;
            ...
            } else if (currentState == 12 && currentSymbol == '_') {
                printf("Tape Accepted!\n");
                return 1;
        ...
        // Dynamically extend the tape if the head moves past its current end
        if (head >= tapeSize) {
            tapeSize *= 2;
            *tape = realloc(*tape, tapeSize);
            if (*tape == NULL) {
                printf("Error: Memory allocation failed.\n");
                exit(1);
            }
            memset(*tape + tapeSize / 2, BLANK, tapeSize / 2);
        }
...
int main() {
    ...
    // Equal
    printf("Tape 1 \n");
    char *tape1 = malloc(INITIAL_TAPE_SIZE);
    strcpy(tape1, "a_"); // Example tape 1
    ...
    // Equal
    printf("Tape 2 \n");
    char *tape2 = malloc(INITIAL_TAPE_SIZE);
    strcpy(tape2, "ba_"); // Example tape 2
    ...
    // Not Equal
    printf("Tape 3 \n");
    char *tape3 = malloc(INITIAL_TAPE_SIZE);
    strcpy(tape3, "baaaabba_"); // Example tape 1
    ...
    // Equal
    printf("Tape 4 \n");
    char *tape4 = malloc(INITIAL_TAPE_SIZE);
    strcpy(tape4, "bbaaaabbba_"); // Example tape 2
